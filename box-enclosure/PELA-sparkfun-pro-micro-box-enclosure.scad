...
module PELA_sparkfun_pro_micro_box_enclosure(l=l, w=w, h=h, bottom_type=bottom_type, top_vents=top_vents, side_holes=side_holes, side_sheaths=side_sheaths, end_holes=end_holes, end_sheaths=end_sheaths, skin=skin, left_wall_enabled=left_wall_enabled, right_wall_enabled=right_wall_enabled, front_wall_enabled=front_wall_enabled, back_wall_enabled=back_wall_enabled, solid_upper_layers=solid_upper_layers, retainer_tab_radius=retainer_tab_radius, center_type=center_type) {
    ...
    difference() {
        union() {
            ...
            board_insertion_space_side_shell(l=l, w=w, h=h, board_l=board_l, board_w=board_w, board_h=board_h, board_thickness=board_thickness);
        }
...
module side_snap_board_holder_cut(flexture_width=flexture_width, side_snap_end_inset=side_snap_end_inset, flexture_width=flexture_width, side_snap_cut_width=side_snap_cut_width, side_snap_cut_depth=side_snap_cut_depth, retainer_tab_radius=retainer_tab_radius, block_height=block_height) {
    ...
    translate([block_width(side_snap_end_inset), block_width(0.5), panel_height(block_height=block_height)]) {        
        ...
        translate([block_width(flexture_width) - side_snap_cut_width, 0, 0]) {
            side_snap_cut(side_snap_cut_width=side_snap_cut_width, cut_height=cut_height, block_height=block_height);
        }

        translate([0, block_width(0.5 - side_snap_cut_depth) - retainer_tab_radius, 0]) {
            side_snap_back_cut(flexture_width=flexture_width, retainer_tab_radius=retainer_tab_radius, cut_height=cut_height, block_height=block_height);
        }
    }
}
...
module side_snap_cut(side_snap_cut_width=side_snap_cut_width, cut_height) {
...
// A vertical slice to allow the flexture to move inward as the board is inserted past the retaining tab
module side_snap_back_cut(flexture_width=flexture_width, retainer_tab_radius=retainer_tab_radius, cut_height) {
