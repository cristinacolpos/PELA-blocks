...
All models include `print-parameters.scad` and `PELA-parameters.scad` directly. Most also include `block.scad` and `technic-block.scad`. When you make changes to these files and save to disk, the preview imnage will update automatically. Press `F6` for a more accurate rendering.
