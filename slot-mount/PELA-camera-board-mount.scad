...
module camera_mount() {
    ...
    translate([block_width(0.5)-skin, block_width(0.5)-skin, 0]) {
        color("red") cube([block_width(l-3)+2*skin, block_width(w-3)+2*skin, block_height()]);
    }
}
    ...
    translate([block_width(-1), block_width(-0.5), 0]) {
        intersection() {
            translate([0, block_width(), 0]) {
                rotate([90, 0, 0]) {
                    PELA_technic_block(l=l, w=1, h=1, sockets=false, knobs=false, panel=false, bolt_holes=false, solid_bottom_layer=true, end_holes=0);
                }
            }
