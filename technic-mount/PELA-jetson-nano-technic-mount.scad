...
top_cut_length = block_width(9);
top_cut_width = block_width(6);
    
top_x = block_width(2.5);
top_y = block_width(-2.5) - top_cut_width;
...
difference() {
    union() {
        ...
        if (render_modules > 0) {
            translate([top_x - edge_thickness, top_y - edge_thickness, -0]) {
                cube([top_cut_length + 2*edge_thickness, top_cut_width + edge_thickness, 1+panel_height(block_height)]);
            }    
        }        
    }
    
    translate([top_x, top_y, -skin]) {
        cube([top_cut_length, top_cut_width, block_height(1, block_height)]);
    }
}
